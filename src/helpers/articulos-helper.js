	...
	try {
		...
		const resultado = await Promise.all(
			...
			result.map(async (articulo) => {
				...
				const [ resultMultimedio ] = await conecction.query( querys.getArticuloFotos, [articulo.id] );
...
const populateArticuloMultimedios = async (result) => {
    try{
		...
		const [ resultMultimedio ] = await conecction.query( querys.getArticuloByIdMultimedios, [result[0].id] );
...
const updateArticuloById = async (nombre, descripcion, categoria_id, id) => {
	try {
		const [ result ] = await conecction.query(querys.editArticulo, [nombre, descripcion, categoria_id, id ]);
		return result;
	} catch (error) {
		throw new Error(error);
	}
