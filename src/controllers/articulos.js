...
const getAll = async (req = request, res = response) => {
  try {
    ...
    const [result, metadata] = await conecction.query(querys.getAllArticulos, [
      limit,
      page,
    ]);
    const resultado = await articulosHelper.populateArticulosFotos(result);
...
const getArticle = async (req = request, res = response) => {
  try {
    ...
    const [result, metadata] = await conecction.query(querys.getArticuloById, [
      id,
    ]);
...
const createArticle = async (req = request, res = response) => {
    try {
      ...
      if (!Array.isArray(multimedios)) multimedios = [req.files.multimedios];
      ...
      const articulo = await articulosHelper.insertArticuloAndGetIt(
        nombre,
        descripcion,
        categoria_id
      );
      ...
      const articuloPopulated = await articulosHelper.populateArticuloMultimedios(
        articulo
      );
...
const editArticle = async (req = request, res = response) => {
  try {
    ...
    await conecction.query(querys.updateArticulo, [
      nombre,
      descripcion,
      categoria_id,
      articulo_id,
    ]);
    const [articulo, metadata] = await conecction.query(
      querys.getArticuloById,
      [articulo_id]
    );
    const articuloPopulated = await articulosHelper.populateArticuloMultimedios(
      articulo
